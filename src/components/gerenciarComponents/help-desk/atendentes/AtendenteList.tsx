...
export default function AtendenteList({
  ...
  useEffect(() => {
    async function getAtendentes() {
      ...
      if (response.ok && response.data) {
        ...
        const ativos = response.data.atendentes.filter(
          (a) => a?.status === true
        );
        setAtendentes(ativos);
      }
    }
    getAtendentes();
  }, []);
  ...
  const openModal = (id:string,nome:string) => {
          ...
          return (
            ...
            >
              ...
              <li className="flex min-w-50 justify-center gap-1">
                <button
                  onClick={()=> openModal(atendente.id, atendente.UsuarioAtendente.nome.slice(0, 20) + "...")}
  ...
  if (modalAtendenteEdit)
    return(
      <div className="h-80 overflow-y-auto">
        <AtendenteEdit atendenteID={atendenteID} atendenteNome={atendenteNome} setModalAtendenteEdit={setModalAtendenteEdit}/>
      </div>
    )
}
