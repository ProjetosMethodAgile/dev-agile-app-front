...
import {  DESATIVA_ATTENDENT_HELPDESK_POR_SETOR_E_ID } from "@/functions/api";
import { TokenData } from "@/types/api/apiTypes";

export async function desativandoAtendentedoSetorHelpdesk(atendenteId:string, setorId:string) {
    try {
        ...
        const result = await DESATIVA_ATTENDENT_HELPDESK_POR_SETOR_E_ID(atendenteId,setorId);
        if (!result?.url) {
