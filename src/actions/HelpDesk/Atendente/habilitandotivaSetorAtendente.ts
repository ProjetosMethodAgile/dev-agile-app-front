...
import { ATIVA_ATTENDENT_HELPDESK_POR_SETOR_E_ID } from "@/functions/api";
import { TokenData } from "@/types/api/apiTypes";

export async function habilitandotivaSetorAtendente(atendenteId:string, setorId:string) {
    try {
        ...
        const result = await ATIVA_ATTENDENT_HELPDESK_POR_SETOR_E_ID(atendenteId,setorId);
        if (!result?.url) {
